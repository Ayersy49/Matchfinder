...
export async function api<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 403 && message.includes('askıya alınmıştır')) {
      alert(message);
      // Opsiyonel: Oturumu kapat
...
// Transfer ownership
export function postTeamTransferOwnership(teamId: string, newOwnerId: string) {
  return api<{ ok: boolean }>(`/teams/${teamId}/transfer-ownership/${newOwnerId}`, {
    method: 'POST',
  });
}
