...
function LoginScreen({ onSuccess }: { onSuccess: () => void }) {
  ...
  try {
    ...
    if (data.devCode) alert("DEV OTP: " + data.devCode); // local test
  } catch (e) {
...
async function verifyOtp() {
  try {
    ...
    if (data.ok) {
      ...
      onSuccess(); // giriş başarılı → sekmeli ekrana geç
    } else {
...
function ProfileScreen() {
  // DB’den oku + kaydet
  const { me, setMe, loading, error, save } = useMe();

  if (loading) {
    return <div className="mx-auto max-w-2xl p-4">Yükleniyor…</div>;
  }
  if (error) {
    return <div className="mx-auto max-w-2xl p-4 text-red-400">❌ {error}</div>;
  }
  if (!me) return null;

  // 3 seçim kuralı ile pozisyon toggle
  function togglePos(key: PositionKey) {
    setMe(prev => {
      if (!prev) return prev;             // me null ise dokunma
      const has = prev.positions.includes(key);
      let positions = has
        ? prev.positions.filter(k => k !== key)
        : [...prev.positions, key];

      if (positions.length > 3) positions = positions.slice(1);
  ...
  async function onSave() {
    if (!me) return;                       // TS için garanti
    try {
      await save({
        dominantFoot: me.dominantFoot,
    ...
    <div className="mx-auto grid max-w-2xl gap-4">
        ...
        <div className="mt-2 flex gap-2">
          {([
            ...
            <button
              key={o.k}
              onClick={() =>
                setMe(v => (v ? { ...v, dominantFoot: o.k } : v))
              }
          ...
          <div className="mt-2 flex items-center gap-3">
            <input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                setMe(v =>
                  v ? { ...v, level: parseInt(e.target.value, 10) } : v
              )
            }
            className="w-full"
          />
          <div className="w-10 rounded-md bg-neutral-800 py-1 text-center text-sm">
            {me?.level ?? 5}
          </div>
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
            {(['GK','LB','CB','RB','LWB','RWB','DM','CM','AM','LW','RW','ST'] as const).map(p => {
            const active = (me?.positions ?? []).includes(p);
            const disabled = !active && (me?.positions?.length ?? 0) >= 3;
            return (
              <button
                key={p}
                type="button"
                ...
                onClick={() => {
                  setMe(v => {
                    if (!v) return v;
                    const has = v.positions.includes(p);
                    let positions = has
                      ? v.positions.filter(k => k !== p)
                      : [...v.positions, p];
                    if (positions.length > 3) positions = positions.slice(1); // en fazla 3
                    return { ...v, positions };
                  });
                }}
                className={`rounded-xl px-3 py-2 text-sm ${
                  active ? 'bg-emerald-500 text-neutral-950' : 'bg-neutral-800'
        ...
        <p className="mt-2 text-xs text-neutral-400">
          Önce üç pozisyon seçin. Fazlasını seçince ilk seçilen çıkar.
        </p>
      </div>
      ...
      <div className="mt-4">
        <button
          ...
          className="rounded-xl bg-emerald-500 px-4 py-2 font-medium text-neutral-950 hover:bg-emerald-400"
        >
          Kaydet
        </button>
    </div>
      ...
      {/* Müsaitlik (demo) – dokunmadık */}
      <section className="rounded-2xl border border-white/10 bg-neutral-900/60 p-4">
        <h3 className="mb-3 text-base font-semibold">Müsaitlik (demo)</h3>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {['Pzt','Sal','Çar','Per','Cum','Cmt','Paz'].map((d) => (
            <div key={d} className="rounded-xl bg-neutral-800 p-3 text-center text-sm">
              {d} 20:00–24:00
            </div>
          ))}
        </div>
        <p className="mt-2 text-xs text-neutral-400">
          Gerçekte haftalık zaman aralıkları ve bugün kapalı/açık ayarı eklenecek.
        </p>
      </section>
    </div>
  );
