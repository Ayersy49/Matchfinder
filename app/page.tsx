...
function MatchesList() {
  ...
  return (
    <div className="grid gap-3">
      {items.map((m) => (
        <div key={m.id} className="rounded-xl border border-white/10 bg-neutral-900/60 p-3">
          <div className="flex items-center justify-between">
            <div className="font-medium">{m.title ?? "Maç"}</div>
            <div className="text-xs text-neutral-400">{m.id}</div>
          </div>
          <div className="mt-1 text-sm text-neutral-300">
            <b>Konum:</b> {m.location ?? "—"} • <b>Seviye:</b> {m.level ?? "—"} •{" "}
            <b>Format:</b> {m.format ?? "—"} • <b>Fiyat:</b>{" "}
          ...
          <div className="mt-2 flex gap-2">
            <a
              href={`/match/${m.id}`}
              className="rounded-xl px-3 py-1 text-sm bg-emerald-600 text-neutral-950 hover:bg-emerald-500"
            >
              Detay / Katıl
            </a>
          </div>
...
function MatchesScreen() {
  // Demo maçlar (id EKLENDİ — /match/{id} sayfasına gidebilmek için)
  const matches = [
    {
      id: "demo-1",
      title: "Beşiktaş - Halısaha",
      time: "Çar 21:00",
      distance: "3.2 km",
      levelText: "Seviye 6–8",
      format: "7v7",
      missing: ["LW", "RB"],
    },
    {
      id: "demo-2",
      title: "Kadıköy - Çetin Emeç",
      time: "Per 20:30",
      distance: "5.0 km",
      levelText: "Seviye 5–7",
      format: "7v7",
      missing: ["ST", "DM"],
    },
    {
      id: "demo-3",
      title: "Mecidiyeköy - Likör",
      time: "Cum 22:15",
      distance: "4.1 km",
      levelText: "Seviye 7–9",
      format: "8v8",
      missing: ["GK"],
    },
  ];
          ...
          >
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <div className="mt-1 text-sm text-neutral-300">
                  {m.time} · {m.distance} · {m.levelText} · {m.format}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() => alert("MVP: Katıl akışı bir sonraki adımda")}
                >
                  Katıl
                </button>
